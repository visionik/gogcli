...
func (c *DocsSedCmd) runPositionalInsert(ctx context.Context, u *ui.UI, account, id string, expr sedExpr) (bool, error) {
	...
	case "^$":
		...
		return true, c.doPositionalInsertWithExpr(ctx, docsSvc, u, id, 1, literalReplacement(expr.replacement), &expr)
	...
	case "$":
		...
		return true, c.doPositionalInsertWithExpr(ctx, docsSvc, u, id, insertIdx, literalReplacement(expr.replacement), &expr)
	}
